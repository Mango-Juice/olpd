...
export const DEEPSEEK_CAMPAIGN_PROMPT_VERSION = "campaign-deepseek-20-persistent-rules";
...
Otherwise return {"status":"ok","body":ProgramNode,"scope":{"mode":"current"|"stage"|"region","region"?:regionID},"guard":boolean,"condition"?:Predicate,"bindings"?:{literalEntityID:{"kind":"public-kind","value":publicKind}}}. Omit null optional fields. scope current means the hero's current region (default for a local command). Use stage only when the player explicitly states a reusable general rule such as “상자가 있으면”, “보일 때마다”, “모든 방에서”, “계속 전진해”, or “항상”; a sentence about the currently visible object remains current. Every entity ID used by a stage rule must have a binding copied from that visible entity's publicKind. Never invent a publicKind, use hidden state, or generalize a literal target. The runtime will resolve each binding only when exactly one visible entity has that publicKind; it never asks AI again. A visibility condition still belongs in the body: use if with a visible predicate, and bind its entity in a reusable stage rule. region is only for a named public region. Every notebook entry is a persistent rule: at every atomic decision the first matching rule in notebook priority is selected again, including after completion. A leading if is tested again; false without else allows lower rules to match. Unconditional movement at higher priority shadows lower rules. Set guard false; all rules participate in priority, not only exceptions. Conditions belong in body as if. Only explicit waiting uses wait/until. Preserve progress within an explicit sequence while it remains selected. Never add completion goals or hidden routing decisions.
ProgramNode grammar:
...
- {"kind":"until","condition":Predicate,"body":Action} — repeat/maintain an ongoing action until its endpoint. An action triggered by a condition uses if, or sequence(wait, action) for explicit waiting, never until: until checks the endpoint first and would skip that action when it is already true. In particular, "release when X" is if(X, release) or sequence(wait(X), release), never until(X, release). An event such as arriving while aboard a drifting object is not an extra move command: preserve the event with wait or if, never invent movement to make the event happen.
Predicate: {"kind":"visible","entity":entityID} for whether the named object appears in world.visible at execution, or {"kind":"property","entity":entityID,"property":publicPropertyKey,"comparison":"eq"|"lt"|"lte"|"gt"|"gte","value":string|number|boolean,"source":"visible"|"remembered"}, or {"kind":"all"|"any","predicates":[Predicate,...]}, or {"kind":"not","predicate":Predicate}. "X가 보이면" means if(visible X, action), with no invented property or wait. visible is a current scene condition, not a remembered property or a prediction that the action is safe.
Verb role contracts:
...
Predicate.property must be an exact key inside an entity properties object (or a current remembered fact). Visibility uses kind visible and world.visible; it is not an entity property. Entity top-level location/parent and actor state are interpretation context, not predicate fields. If an explicitly requested condition cannot be represented by this grammar, ask for clarification; never invent a location/arrived field or silently drop the condition. Use only listed entity/actor IDs and public properties. Output JSON, not prose or code fences.`;
export function deepSeekPublicWorld(world: WorldState) {
  ...
  return {
    stageId: world.stageId, segmentId: world.segmentId, actors, entities, visible: world.visible,
...
function validateReferences(program: InstructionProgram, world: WorldState): void {
  ...
  function predicate(node: Predicate, observedEarlier: ReadonlySet<string>): void {
    ...
    if (node.kind === "visible") { entity(node.entity); if (!visible.has(node.entity)) uncertain("보임 조건의 대상이 현재 공개된 물체와 일치하지 않아요."); return; }
...
function referencedEntityIds(program: InstructionProgram): Set<string> {
  ...
  function predicate(node: Predicate): void {
    ...
    if (node.kind === "property" || node.kind === "visible") ids.add(node.entity);
  }
