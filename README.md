...
- 진행과 메모는 자동 저장한다. 개편 전 진행 중 기록은 별도 복구 자료로 보존하고, 완료한 모험은 클리어·해금·최고 기록만 유지한다. 다중 창 충돌은 멈춰 알린다.
- 1장의 기존 진행 저장(`layoutVersion` 없음/1)은 옛 도입과 8방 경로로 계속한다. 새 시작·초기화는 6장면(`layoutVersion: 3`)을 사용하고, 완료한 모험의 상세 기록은 누적하지 않는다.
- 로컬 QA: **http://localhost:5173/?qa=1**. 장과 개별 스테이지를 바로 선택하며 일반 진행과 따로 저장한다.

## 진행과 최근 행동

현재 메모와 진행은 자동 저장하고, **방금 무슨 일이?**에서 현재 플레이의 행동을 확인한다. 장을 완료하면 현재 실행본 대신 장별 완료 여부·해금·최고 기록과 마지막 완료 참조/시각만 저장한다. 지난 모험 목록이나 완료 회차 전체 재생은 제공하지 않는다.

캠페인 저장 문서 v5는 이전 상세 보관본을 검증해 요약으로 전환한다. 새 문서 저장이 성공한 뒤에만 이전 완료 상세 기록을 정리한다. 진행 중인 메모·현재 행동 기록·개편 전 활성 복구 자료는 유지한다. 이전 완료 이력으로 되돌리는 기능은 없다.
...
- `src/components/CampaignChronicle.tsx`: 현재 플레이의 행동 설명·장면 재생
