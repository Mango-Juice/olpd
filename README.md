...
아래 10장·66스테이지 캠페인은 로컬 구현 상태다. 이번 작업에서는 배포하지 않았다. [현재 구현](docs/spatial-implementation.md)을 참고한다.
...
- 메모는 위쪽부터 우선한다. 적용할 지시가 없거나 실행을 마치면 제자리에서 멈춘다. 계속 걷게 하려면 전진 지시를 남겨야 하며, 시스템이 이동·상호작용·역할을 대신 결정하지 않는다.
