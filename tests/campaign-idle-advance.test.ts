...
describe("authored idle advance", () => {
  ...
  it("runs true guards first and lets both false and unknown dormant guards yield to idle movement", () => {
    let idleCalls = 0;
    ...
    expect(guardedStop.world.actors.hero.location.x).toBe(1);
    expect(guardedStop.events.find((event) => event.actor === "hero")?.instructionId).toBeNull();
  ...
  it("does not let a dormant condition freeze authored idle movement", () => {
    let idleCalls = 0;
    ...
    expect(waiting.world.actors.hero.location.x).toBe(1);
    ...
    expect(stopped.world.actors.hero.location.x).toBe(1);
    expect(idleCalls).toBe(2);
  });
  ...
  it("keeps the fatal idle result visible and grants exactly one post-death write", () => {
    for (const source of ["execute", "environment"] as const) {
      ...
      expect(result.world).not.toEqual(world);
      expect(result.world.actors.hero.location.x).toBe(1);
      expect(result.notebook).toMatchObject({ deaths: 1, canWrite: true, canDelete: true });
      expect(result.events.some((event) => event.outcome === "failure")).toBe(true);
      expect(environmentCalls).toBe(source === "execute" ? 0 : 1);
    }
  });

  it("keeps 02-1 riding until an explicit dismount instruction arrives", () => {
