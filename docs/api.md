...
[공통 메모 수칙](game-rules.md)에 따라 모든 메모를 매 행동 경계에서 우선순위대로 재평가한다. `guard`는 이전 저장과의 호환 필드이며 우선순위 권한을 추가하지 않는다. 새 해석은 `guard: false`를 사용하고 조건은 `body.if`로 표현한다. `completed`는 수칙 소모 기록으로 사용하지 않는다. 한 줄 내부의 명시적 순서와 기다림은 유지한다.
