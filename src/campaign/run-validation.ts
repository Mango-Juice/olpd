...
import { parseProgram } from "./validation";
...
function cursorMatches(node: ProgramNode, position: ProgramCursor, path: number[] = []): boolean {
  ...
  if (position.status === "pending" && (position.index !== 0 || position.children.length !== 0 || position.branch !== null)) return false;
  if (node.kind === "action" || node.kind === "wait" || node.kind === "until") {
    return position.children.length === 0 && position.branch === null && (node.kind === "until" ? node.body.verb !== "hold" || position.index <= 1 : position.index === 0);
  }
  if (node.kind === "if") {
...
export function parseStageRun(value: unknown): StageRun | null {
  if (!record(value) || value.version !== 2 || !Array.isArray(value.waitingStates) || !value.waitingStates.every((item) => typeof item === "string") || !name(value.id) || !isStageId(value.stageId) || value.stageId === 1 || !integer(value.revision) || !(value.statusReason === null || typeof value.statusReason === "string") || !["bookmark", "running", "waiting", "blocked", "failed", "cleared"].includes(String(value.phase)) || !world(value.world) || !world(value.checkpoint) || !record(value.world) || !record(value.checkpoint) || value.world.stageId !== value.stageId || value.checkpoint.stageId !== value.stageId || !names(value.clearedSegments) || value.clearedSegments.length > stageSummary(value.stageId).segments || !integer(value.seal) || value.seal > 2 || (value.stageId !== 10 && value.seal !== 0)) return null;
  const book = value.notebook;
  if (!record(book) || !Array.isArray(book.instructions) || book.instructions.some((item) => parseProgram(item) === null) || typeof book.canWrite !== "boolean" || typeof book.departed !== "boolean" || typeof book.editing !== "boolean" || !integer(book.erasers) || book.erasers > 2 || !integer(book.bells) || !names(book.visitedBookmarks)) return null;
