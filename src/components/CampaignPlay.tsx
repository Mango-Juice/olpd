...
import { stageDynamics } from "../campaign/level";
...
import type { InstructionProgram, Predicate, ProgramNode, WorldState } from "../campaign/types";
...
export function describeProgram(world: WorldState, node: ProgramNode): string {
  switch (node.kind) {
    case "action": return `${node.actor === "keeper" ? "등지기" : "용사"}: ${targetName(world, node.target)}${node.destination ? ` → ${targetName(world, node.destination)}` : ""} ${VERBS[node.verb]}${node.instrument ? ` (${targetName(world, node.instrument)} 사용)` : ""}${node.amount !== undefined ? ` ${node.amount}칸` : ""}`;
...
export function CampaignPlay({ run, stage, settings, onCommit, interpret, onRoadmap, onPractice, archivedPrograms = [], practice = false, onSettingsChange }: CampaignPlayProps) {
  ...
  live.current = run;
  const segment = stage.segments.find((item) => item.id === run.world.segmentId);
  ...
  async function submit(event: FormEvent) {
    ...
    if (!run.notebook.canWrite || !run.notebook.editing || !count || count > CAMPAIGN_INPUT_LIMIT || saving || pending) return;
    cancel();
  ...
  async function confirm() {
    ...
    try {
      const notebook = writeProgram(run.notebook, candidate, editingId);
      ...
      if (await commit({ ...run, notebook, revision: run.revision + 1 })) { setDraft(""); setCandidate(null); setEditingId(undefined); }
  ...
  const editCost = (id: string) => correctionId === id ? "무료 확인" : run.notebook.erasers > 0 ? "지우개 1개" : "종 3회";
  return <div className="shell campaign-play">
    <PlayHeader actions={<>
    ...
    <PlayLayout>
      <section className="game-panel" aria-label="던전 플레이">
        <div className="scene-frame campaign-scene">
          <CampaignCanvas world={run.world} title={segment.title} reducedMotion={settings.reducedMotion} paused={paused || run.phase === "bookmark" || run.phase === "cleared"} selectedEntityId={selected} onSelectEntity={setSelected} />
          <p className="campaign-goal"><span>이번 목표</span>{segment.goal}</p>
        </div>
    {!practice ? <PlayStageProgress chapter={stage.id} stages={stage.segments} currentId={run.world.segmentId} completedIds={run.clearedSegments} /> : null}
        <p className="campaign-learning">{campaignLearningNote(run)}</p>
        <p role="status" className="campaign-status">{saving ? "기억을 보관하고 있어요…" : notice || (run.phase === "bookmark" && run.clearedSegments.length > 0 ? `${run.world.segmentId.replace(/^0/, "")} · ${segment.title}. 다음 스테이지에서 한 줄을 더 남길 수 있어요.` : run.statusReason) || ""}</p>
        {run.notebook.canWrite && run.notebook.editing ? <PlayComposer onSubmit={submit}>
          <label htmlFor="campaign-instruction">{editingId ? "메모를 고쳐 쓰기" : "이번 생에 남길 한 줄"}</label>
          ...
          <textarea id="campaign-instruction" value={draft} rows={2} placeholder="용사가 할 행동을 한 줄로 적어 주세요." disabled={saving} onChange={(event) => { cancel(); setDraft(event.target.value); }} />
        ...
        <div className="campaign-controls">
          {run.phase === "bookmark" && run.notebook.instructions.length > 0 ? <button type="button" className="primary" disabled={saving || pending || !!candidate || !!correctionId} onClick={() => { cancel(); setPaused(false); void commit(departStage(run)); }}>이 메모로 출발 →</button> : null}
          ...
          {run.notebook.departed && run.phase !== "cleared" ? <button type="button" className="secondary" disabled={saving || !!correctionId} onClick={() => { cancel(); setPaused(false); void commit(rewindStage(run)); }}>{practice ? "연습 처음으로 · 무료" : "종 1회 · 되감고 한 줄 쓰기"}</button> : null}
        </div>
        <PlayHints hints={segment.hints} resetKey={`${run.id}:${segment.id}`} />
        <details className="campaign-tools"><summary>플레이 도구</summary><label><input type="checkbox" checked={fast} onChange={(event) => setFast(event.target.checked)} /> 빠르게 재생</label><button type="button" className="subtle" disabled={saving || run.notebook.departed} onClick={() => { cancel(); onPractice(); }}>비용 없는 연습</button>{!run.notebook.departed ? <button type="button" className="subtle" disabled={saving || !!correctionId} onClick={() => { cancel(); setPaused(false); void commit(rewindStage(run)); }}>{practice ? "연습 처음으로 · 무료" : "종 1회 · 되감고 한 줄 쓰기"}</button> : null}</details>
        <div className="stats"><strong>{run.notebook.bells}</strong><span>{practice ? "연습 · 본편 기록에 남지 않아요" : "종 · 되감은 횟수"}</span></div>
      ...
      <PlayNotebook count={run.notebook.instructions.length}>
        ...
        <ol className="campaign-notes instruction-list">{run.notebook.instructions.map((program, index) => <li key={program.id} className={`instruction ${run.execution.active?.instructionId === program.id ? "active" : ""}`}><p>{program.text}</p><details className="campaign-note-tools"><summary>메모 관리</summary><button type="button" aria-label={`${program.text} 위로`} disabled={!run.notebook.editing || saving || index === 0} onClick={() => void changeNotebook(() => reorderProgram(run.notebook, program.id, index - 1))}>↑</button><button type="button" aria-label={`${program.text} 아래로`} disabled={!run.notebook.editing || saving || index === run.notebook.instructions.length - 1} onClick={() => void changeNotebook(() => reorderProgram(run.notebook, program.id, index + 1))}>↓</button><button type="button" disabled={!run.notebook.editing || !run.notebook.canWrite || saving || (!!correctionId && correctionId !== program.id)} onClick={() => { cancel(); setDraft(program.text); setEditingId(program.id); }}>수정 · {editCost(program.id)}</button><button type="button" disabled={!run.notebook.editing || saving || (!!correctionId && correctionId !== program.id)} onClick={() => void changeNotebook(() => deleteProgram(run.notebook, program.id))}>삭제 · {editCost(program.id)}</button></details></li>)}</ol>
        {!run.notebook.instructions.length ? <p className="empty-note">아직 남긴 한 줄이 없어요.<br />첫 번째 기억을 남겨 주세요.</p> : null}
        <div className="paper-foot"><span>남은 지우개</span><div className="erasers" aria-label={`지우개 ${run.notebook.erasers}개`}>{[0, 1].map((index) => <span key={index} className={`eraser ${index >= run.notebook.erasers ? "spent" : ""}`} />)}</div></div>
        <p className="paper-note">지우개를 다 쓰면, 한 줄을 지울 때 종 3회.</p>
        {!run.notebook.departed && archivedPrograms.length ? <details><summary>지난 장의 메모 가져오기 · 무료</summary>{archivedPrograms.map((program) => <button type="button" key={program.id} className="subtle" disabled={saving} onClick={() => void changeNotebook(() => copyArchivedProgram(run.notebook, program))}>{program.text}</button>)}</details> : null}
      </PlayNotebook>
    </PlayLayout>
